...
When the question allows it, the bot looks for more than one relevant kind of source before concluding, because one kind of source carries one kind of bias. It will not pad the brief with a loosely related page just to add variety, though. If a topic only has one kind of source, such as a set of independent official filings, it continues, notes that limitation under `Gaps`, and still judges the evidence by independent corroboration rather than by how many source types it collected. Restrict it to certain platforms and it obeys, then records under `Gaps` what the restriction left out. `Searches run` names only the platforms it actually used.
