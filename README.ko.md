...
각 역할마다 서로 다른 Grok Bot을 만들어 사용하세요. 결과물을 직접 복사하지 않고도 확인하고 전달을 승인할 수 있도록 생산 역할들은 하나의 그룹 대화에 함께 두고, PR Verifier는 그 대화 밖에 두세요. 결과물이 그룹에 올라온다고 해서 다음 봇이 작업을 시작하지는 않습니다.
...
- **전달과 승인을 구분합니다.** 현재 대화에 결과나 부족한 입력에 관한 안내를 남기는 일에는 승인이 필요하지 않습니다. 그러나 다른 봇에게 다음 작업을 요청하는 일에는 승인이 필요하고, 그룹 대화에서는 특정 봇을 지목하지 않은 요청도 참여 봇이 스스로 응답할 수 있으므로 그것 역시 승인을 받아야 합니다. 또한 봇은 사용자가 직접 요청하거나 자신을 지목한 명시적인 전달을 받았을 때만 작업을 시작하므로, 그룹에 남겨진 통지만으로는 움직이지 않습니다. 다만 전달 행위는 전달받은 봇의 프로필이 요구하는 사람 승인 조건을 대신하지 못합니다. 그래서 PR Producer는 구현에 착수하기 전에 사용자의 승인을 다시 받아야 하지만, PR Verifier처럼 별도의 사람 승인 조건이 없는 읽기 전용 역할은 승인된 전달을 받은 즉시 검증을 시작할 수 있습니다.
