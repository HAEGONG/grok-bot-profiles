...
바로 사용할 수 있는 Grok Bot 프로필 모음입니다. 각 프로필은 봇이 수행할 하나의 명확한 역할과 구체적인 결과물을 정의하고, 봇의 권한이 끝나는 지점을 명시합니다.

범용 에이전트를 사용하면 책임의 경계가 불분명해질 수 있습니다. 이 모음은 명세 작성, 조사, 구현, 검증을 분리하여 봇이 자신의 작업을 직접 승인하지 못하게 합니다.
...
2. 아래 표에서 사용할 프로필의 **설정 프롬프트** URL을 복사합니다.
3. 복사한 URL을 봇의 첫 메시지로 붙여 넣습니다.
4. 봇이 요청하면 GitHub를 연결합니다.
5. 해당 프로필의 README에 있는 **첫 작업**을 봇에게 보냅니다.

`SETUP.md`는 프로필을 가져와 봇의 Name, Title, Description을 설정합니다. `PROFILE.md`의 내용을 Description에 직접 붙여 넣지 마세요.
...
| 봇 | 다음 작업을 수행하기 전에 중단해야 합니다 |
| --- | --- |
| Spec Writer | 명세를 승인하거나 구현하는 작업 |
| Bug Reproducer | 코드를 수정하거나 코딩 에이전트를 호출하는 작업 |
| PR Producer | Pull Request를 검토하거나 승인하거나 병합하는 작업 |
| PR Verifier | 수정 사항을 구현하거나 Pull Request를 병합하는 작업 |

PR Producer와 PR Verifier에는 같은 봇이나 같은 대화를 사용하면 안 됩니다. 두 역할의 분리는 구현 방식이 아니라 승인 경계입니다.
...
| 파일 | 용도 |
| --- | --- |
| `PROFILE.md` | 봇의 지속적인 정체성, 역할, 결과물의 형식과 조건, 작업 규칙, 승인 경계를 정의합니다 |
| `SETUP.md` | 최초 한 번만 사용하는 설정 메시지와 작업별 실행 체크리스트를 제공합니다 |
| `README.md` | 설치 방법, 필요한 연동 서비스, 관련 봇, 첫 작업을 안내합니다 |

`PROFILE.md`의 YAML 프런트매터는 Grok Bot 인터페이스의 다음 필드에 대응합니다.

| 앱 필드 | 프로필에서 가져오는 값 |
| --- | --- |
| Name | `name` |
| Title | `title` |
| Description | YAML 프런트매터 아래의 Markdown 본문 |
...
- **Description에는 지속적으로 적용할 내용을 작성합니다.** 항상 적용할 역할 규칙은 `PROFILE.md`에 작성하고, 설정 방법과 특정 작업에만 필요한 지침은 다른 문서에 작성합니다.
- **실패를 명시적으로 표현합니다.** 봇이 상황을 추측하거나 권한을 임의로 확대하지 않도록 `BLOCKED`, `HOLD`처럼 이름이 있는 결과를 사용합니다.
- **직접 확인할 수 있는 근거를 요구합니다.** 로그, 테스트 결과, 검사 상태, URL, 저장소 상태를 지어내면 안 됩니다.
- **권한 경계에서 중단합니다.** Pull Request를 열고, 승인하고, 병합하고, 배포하는 작업은 서로 다른 권한입니다.

결과물, 정보 출처, 도구, 일정, 승인 경계 중 하나라도 달라진다면 별도의 봇을 만드세요.
...
이 프로필을 사용하여 설정에 드는 시간을 줄였다면 저장소에 별을 추가하고, 유용했던 작업 흐름을 다른 사람과 공유해 주세요.
